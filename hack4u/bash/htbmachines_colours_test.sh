...
function searchMachine (){
  machineName="$1"
#  infoMachine=cat bundle.js | awk "/name: \"$machineName\"/,/resuelta:/" | grep -vE "id:|sku:|resuelta" | tr -d '"' | tr -d ',' | sed 's/^ *//'
  ...
  for i in cat bundle.js | awk "/name: \"$machineName\"/,/resuelta:/" | grep -vE "id:|sku:|resuelta" | tr -d '"' | tr -d ',' | sed 's/^ *//'; do
    echo -e "   ${yellowColour}" + awk '{print }'${endColour}${grayColour} : $part2 ${endColour}"
  done
}
